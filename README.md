...
- Сохранение токена в `localStorage` через `authStorage`.
...
После успешного входа или регистрации API возвращает JWT. Frontend сохраняет его через `setToken` и перенаправляет пользователя на `/MainNotes`.

Меню профиля доступно авторизованному пользователю. Кнопка открывает `ProfilePopover`, а пункт выхода удаляет JWT и возвращает пользователя на `/login`.
